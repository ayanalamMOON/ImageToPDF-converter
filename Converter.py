...
def start_conversion(input_entry, output_entry, quality_entry, progress_bar, status_label, recursive=True, min_date=None, skip_converted=True, delete_source=False, pdf_options=None):
    ...
    heic_to_pdf_with_fallback(input_folder, output_pdf, compression_quality, supported_formats, progress_bar, status_label, recursive, min_date, skip_converted, delete_source, pdf_options)
...
def create_gui():
    ...
    tk.Checkbutton(options_frame, text="Scan subfolders", 
                   variable=recursive_var).pack(side=tk.LEFT, padx=5)
    
    skip_converted_var = tk.BooleanVar(value=True)
    tk.Checkbutton(options_frame, text="Skip converted files", 
                   variable=skip_converted_var).pack(side=tk.LEFT, padx=5)
    
    delete_source_var = tk.BooleanVar(value=False)
    tk.Checkbutton(options_frame, text="Delete source files", 
                   variable=delete_source_var).pack(side=tk.LEFT, padx=5)
    ...
    ttk.Radiobutton(pdf_frame, text="Portrait", variable=orientation_var, 
                    value="P").pack(side=tk.LEFT, padx=5)
    ttk.Radiobutton(pdf_frame, text="Landscape", variable=orientation_var, 
                    value="L").pack(side=tk.LEFT, padx=5)
    
    # Page size
    tk.Label(pdf_frame, text="Page Size:").pack(side=tk.LEFT, padx=5)
    size_var = tk.StringVar(value="A4")
    size_combo = ttk.Combobox(pdf_frame, textvariable=size_var, 
    ...
    ttk.Checkbutton(pdf_frame, text="Page Numbers", 
                    variable=page_numbers_var).pack(side=tk.LEFT, padx=5)
    ...
    ttk.Checkbutton(merge_frame, text="Merge PDFs", 
                    variable=merge_var).pack(side=tk.LEFT)
    ttk.Button(merge_frame, text="Add PDFs", 
               command=add_pdf_to_merge).pack(side=tk.LEFT)
